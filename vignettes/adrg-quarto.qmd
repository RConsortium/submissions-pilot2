...
::: callout-note
After the restore process completes, the R console may give a warning message about packages such as `teal` not being installed from a CRAN-like repository. At the time of completing this Pilot, the `teal` and `teal.data` R packages were not released to CRAN. Instead, they were installed from a custom repository hosted on `insightsengineering.github.io`.
:::
